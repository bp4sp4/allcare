...
# 기관(training_institution) 좌표 일괄 변환 스크립트
...
while true; do
  ...
  echo "  기관: 총 ${INSTITUTION_TOTAL:-0}건 처리 → 성공 ${INSTITUTION_SUCCESS:-0}건 / 실패 ${INSTITUTION_FAILED:-0}건"
  ...
  if [ "${INSTITUTION_SUCCESS:-0}" -eq 0 ] && [ ! -z "$INSTITUTION_FAILED" ]; then
